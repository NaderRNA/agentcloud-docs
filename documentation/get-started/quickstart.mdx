...
<AccordionGroup>
  ...
  <Accordion icon="rectangle-terminal" title="Run install.sh in terminal">
    <Warning>
    For now there is a reliance on GCP for Bucket storage to store files for RAG. Make sure you have your GCP Project ID ready and you have downloaded a service account keyfile.json with a with ```Storage Admin``` role to your machine [(instructions here).](https://cloud.google.com/iam/docs/keys-create-delete)
    We will add AWS S3 and Local Disk storage in future releases so that we can remove the dependancy on GCP on install.
    </Warning>
