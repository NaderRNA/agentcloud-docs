...
To ensure optimal performance when running AgentCloud, we recommend a minimum of 16GB of RAM. However, please note that the actual memory usage may vary depending on factors such as the size of your dataset and the complexity of your projects. While testing, we typically observe memory usage ranging from 8GB to 10GB, which includes the operating system and other concurrently running programs within Docker. Therefore, it's essential to consider your available free memory when provisioning your system for running AgentCloud.
